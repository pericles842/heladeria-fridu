<div id="menu" class="d-flex flex-column flex-shrink-0 p-3 " style="width: 250px;height: 100vh">
    <!-- EL LOGO DEL MENU -->
    <a onclick="touchMenu(0)" href="/" class="d-flex align-items-center justify-content-center mb-3 mb-md-0 me-md-auto link-dark text-decoration-none">
        <img style="width: 35%;  object-fit: cover; object-position: center;" src="../img/logo.jpg">
    </a>
    <hr>
    <!-- SECCIONES DEL MENU DE NAVEGACIÓN -->
    <ul class="nav nav-pills flex-column gap-1 mb-auto">

        <li class="nav-item">
            <a onclick="touchMenu(1)" href="#" class="nav-link active" aria-current="page">
                <i class="bi bi-people-fill"></i>
                <svg class="bi pe-none me-2" width="16" height="16">
                    <use xlink:href="#home"></use>
                </svg>
                Roles y Permisos
            </a>
        </li>

        <li>
            <a onclick="touchMenu(2)" href="#" class="nav-link  link-dark">
                <i class="bi bi-person-add"></i>
                <svg class="bi pe-none me-2" width="16" height="16">
                    <use xlink:href="#speedometer2"></use>
                </svg>
                Clientes
            </a>
        </li>

        <li>
            <a onclick="touchMenu(3)" href="#" class="nav-link  link-dark">
                <i class="bi bi-tags"></i>
                <svg class="bi pe-none me-2" width="16" height="16">
                    <use xlink:href="#table"></use>
                </svg>
                Proveedores
            </a>
        </li>

        <li>
            <a onclick="touchMenu(4)" href="#" class="nav-link link-dark">
                <i class="bi bi-box-seam"></i>
                <svg class="bi pe-none me-2" width="16" height="16">
                    <use xlink:href="#grid"></use>
                </svg>
                Almacenen
            </a>
        </li>

        <li>
            <a onclick="touchMenu(5)" href="#" class="nav-link link-dark">
                <i class="bi bi-boxes"></i>
                <svg class="bi pe-none me-2" width="16" height="16">
                    <use xlink:href="#people-circle"></use>
                </svg>
                Productos
            </a>
        </li>

        <li>
            <a onclick="touchMenu(6)" href="#" class="nav-link link-dark">
                <i class="bi bi-bar-chart-fill"></i>
                <svg class="bi pe-none me-2" width="16" height="16">
                    <use xlink:href="#people-circle"></use>
                </svg>
                Categoría
            </a>
        </li>

        <li>
            <a onclick="touchMenu(7)" href="#" class="nav-link link-dark">
                <i class="bi bi-menu-button-wide-fill"></i>
                <svg class="bi pe-none me-2" width="16" height="16">
                    <use xlink:href="#people-circle"></use>
                </svg>
                Inventario
            </a>
        </li>

        <li>
            <a onclick="touchMenu(8)" href="#" class="nav-link link-dark">
                <i class="bi bi-truck"></i>
                <svg class="bi pe-none me-2" width="16" height="16">
                    <use xlink:href="#people-circle"></use>
                </svg>
                Despacho
            </a>
        </li>

        <li>
            <a onclick="touchMenu(9)" href="#" class="nav-link link-dark">
                <i class="bi bi-folder2-open"></i>
                <svg class="bi pe-none me-2" width="16" height="16">
                    <use xlink:href="#people-circle"></use>
                </svg>
                Reportes
            </a>
        </li>
    </ul>
    <hr>
    <a onclick="touchMenu(10)" href="#" class="nav-link link-dark">
        <i class="bi bi-box-arrow-left text-danger"></i>
        <span class="p-4">Salir</span>
    </a>
</div>
<script>
    /** 
     * CAMBIA EL COLOR DE MENU
     * @param {number} index indice del menu 
     * @autor Louis
     */
    function touchMenu(index) {
        const links = document.querySelectorAll('#menu a');

        links.forEach((link, i) => {
            if (i === index) {
                link.classList.add('active');
                link.classList.remove('link-dark');
            } else {
                link.classList.remove('active');
                link.classList.add('link-dark');
            }
        });

    }
</script>